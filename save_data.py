...
def save_data(url):
    ...
    try:
        df1 = pd.read_excel(f'raw_data/raw_{team_name_1}.xlsx')
        data_team_1[0] = list(df1['match_number'])[-1] + 1
    except:
        df1 = pd.DataFrame(columns=columns)
    
    try:
        df2 = pd.read_excel(f'raw_data/raw_{team_name_2}.xlsx')
    ...
    df1.to_excel(f'raw_data/raw_{team_name_1}.xlsx', index=False)
    df2.to_excel(f'raw_data/raw_{team_name_2}.xlsx', index=False)
    print('---Saved data---\n')
